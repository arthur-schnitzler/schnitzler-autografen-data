...
def build_corresp_action(action_type, persons, places, by_display, by_name, place_lookup,
...
def build_corresp_desc(entry, by_display, by_name, place_lookup):
    ...
    corresp_sent = build_corresp_action(
        "sent", senders, places, by_display, by_name, place_lookup,
        date_start, date_end)

    corresp_received = build_corresp_action(
        "received", receivers, [], by_display, by_name, place_lookup)
...
def build_tei(entry, by_display, by_name, place_lookup):
    eid = entry["id"]
    ...
    # title level="a"
    title_a = xml_escape(entry.get("title", ""))
    ...
    if etype == "brief":
        profile_content = (
            ...
            f"         </langUsage>\n"
            f"{build_corresp_desc(entry, by_display, by_name, place_lookup)}")
    else:
        profile_content = (
            f"         <langUsage>\n"
            f'            <language ident="de-AT">German</language>\n'
...
def main():
    ...
    for json_file in JSON_FILES:
        ...
        for entry in data:
            ...
            tei_xml = build_tei(entry, by_display, by_name, place_lookup)
